  ...
  test("capture products page", async ({ page }) => {
    ...
    try {
      await page.waitForSelector(
        '[class*="product"], [class*="card"], h1, h2',
        { timeout: 5000 },
      );
    } catch {
...
test.describe("06 - Theme Toggle", () => {
  ...
  test("capture light mode dashboard", async ({ page }) => {
    ...
    if ((await themeToggle.count()) > 0) {
      ...
      const toggleArea = page
        .locator("text=Dark Mode, text=Light Mode")
        .first();
  ...
  test("capture sidebar", async ({ page }) => {
    ...
    if (await sidebar.isVisible()) {
      await sidebar.screenshot({
        path: `${SCREENSHOT_DIR}/15b_sidebar_only.png`,
      });
    }
  });
});
